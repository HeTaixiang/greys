...
DEFAULT_OUTPUT_PATH="$HOME/trace.log"
...
usage()
{
    echo "
greys usage:
    the format was <PID>[@IP:PORT] <COMMAND ARGS> [OUTPATH]
     <PID> : the target Java Process ID
      [IP] : the target's IP
    [PORT] : the target's PORT
    <COMMAND ARGS> : the command to run in local
    [OUTPATH] : the output log file

example:
    ./greys-trace.sh <PID> <\"-n 2 *StringUtils isBlank\"> [~/xxx.log]
"
}
...
parse_arguments()
{
    ...
    [ -z ${OUTPUT_PATH} ] && OUTPUT_PATH=${DEFAULT_OUTPUT_PATH}
