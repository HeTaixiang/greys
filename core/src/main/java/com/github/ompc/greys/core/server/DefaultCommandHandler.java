...
public class DefaultCommandHandler implements CommandHandler {
    ...
    private final Server gaServer;
    private final Instrumentation inst;

    public DefaultCommandHandler(Server gaServer, Instrumentation inst) {
    ...
    public void executeCommand(final String line, final Session session) throws IOException {
        ...
        final WritableByteChannel socketChannel = session.getChannel();
            ...
            while (!session.isDestroy()
                ...
                try {
                    ...
                    // 读出了点东西
                        ...
                        write(session.getChannel(), segment, session.getCharset());
                    }
     ...
     * 绘制提示符
     */
    private void reDrawPrompt(Session session, WritableByteChannel socketChannel, Charset charset, String prompt) throws IOException {
        if (!session.isSilent()) {
     ...
     * 输出到网络
     */
    private void write(WritableByteChannel socketChannel, String message, Charset charset) throws IOException {
        write(socketChannel, charset.encode(message));
    }

    private void write(WritableByteChannel channel, ByteBuffer buffer) throws IOException {
