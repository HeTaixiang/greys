...
public class GreysLauncher {
    ...
    public static final String DEFAULT_OUT_PATH = getProperty("user.dir") + separator + "greys.log";
    ...
    private Configure analyzeConfigure(String[] args) {
        ...
        parser.accepts("remote").withOptionalArg().ofType(boolean.class);
        parser.accepts("command").withOptionalArg().ofType(String.class);
        parser.accepts("outPath").withOptionalArg().ofType(String.class);
        ...
        if (os.has("remote")) {
            configure.setRemote((Boolean)os.valueOf("remote"));
        } else {
            configure.setRemote(true); //默认C/S模式，开启网络服务
        }
        if (!configure.isRemote() && os.has("command")) {
            configure.setCommand((String)os.valueOf("command"));
            if (os.has("outPath")) {
                String outPath = (String) os.valueOf("outPath");
                if (outPath.trim().length() > 0) {
                    configure.setOutPath(outPath);
                }else {
                    configure.setOutPath(DEFAULT_OUT_PATH);
                }
            }else {
                configure.setOutPath(DEFAULT_OUT_PATH);
            }
