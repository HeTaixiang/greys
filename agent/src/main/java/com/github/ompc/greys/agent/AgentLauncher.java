...
public class AgentLauncher {
    ...
    private static synchronized void main(final String args, final Instrumentation inst) {
        try {
            ...
            final Class<?> classOfServerFactory = agentLoader.loadClass("com.github.ompc.greys.core.server.ServerFactory");

            final Object objectOfServerFactory = classOfServerFactory.newInstance();
...
//            final int javaPid = (Integer) classOfConfigure.getMethod("getJavaPid").invoke(objectOfConfigure);
            ...
            final Class<?> classOfServer = agentLoader.loadClass("com.github.ompc.greys.core.server.Server");

            final Object objectOfServer = classOfServerFactory.getMethod("getServer", classOfConfigure, Instrumentation.class)
                    .invoke(objectOfServerFactory, objectOfConfigure, inst);
...
//            final boolean isBind = (Boolean) classOfGaServer.getMethod("isBind").invoke(objectOfGaServer);
            final boolean isBind = (Boolean) classOfServer.getMethod("isBind").invoke(objectOfServer);
            if (!isBind) {
                try {
//                    classOfGaServer.getMethod("bind", classOfConfigure).invoke(objectOfGaServer, objectOfConfigure);
                    classOfServer.getMethod("bind", classOfConfigure).invoke(objectOfServer, objectOfConfigure);
                } catch (Throwable t) {
//                    classOfGaServer.getMethod("destroy").invoke(objectOfGaServer);
                    classOfServer.getMethod("destroy").invoke(objectOfServer);
                    throw t;
                }
